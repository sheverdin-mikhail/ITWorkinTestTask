...
import { Web3ContextScheme } from '../types/web3';
import getProvider from '@/shared/ui/lib/web3Provider';
...
const Web3Provider: React.FC<Web3ProviderProps> = ({ children }) => {
    const [state, setState] = useState<Web3ContextScheme>({ ...INITIAL_STATE });

    const connectAccountHandler = useCallback(async () => {
        try {
            // Запросить подключение к Metamask
            ...
            localStorage.setItem(import.meta.env.VITE_USER_ADDRESS_LOCALSTORAGE_KEY, accountAddress)

            // Получить баланс в BNB
            const balanceBNB = await provider.getBalance(accountAddress);
            setState(prev => ({
                ...prev,
                balance: formatEther(balanceBNB)
            }));
        } catch (error) {
    ...
    const disconnectAccountHandler = useCallback(async () => {
       setState(prev => ({
        ...prev,
        address: ''
       }))
       localStorage.removeItem(import.meta.env.VITE_USER_ADDRESS_LOCALSTORAGE_KEY)
    }, [])
    ...
    const fetchCurrentEpoch = useCallback(async () => {
        try {
          ...
          setState(prev => ({
            ...prev,
            currentEpoch: formatUnits(epoch, 0)
          }));
        } catch (error) {
          console.error('Ошибка получения currentEpoch', error);
        }
      }, [])
    ...
    useEffect(() => {
        const address = localStorage.getItem(import.meta.env.VITE_USER_ADDRESS_LOCALSTORAGE_KEY)
        if ( address ) {
            provider.getBalance(address).then((balanceBNB) => {
                setState(prev => ({
                    ...prev,
                    address: address,
                    balance: formatEther(balanceBNB)
    ...
    },[state.balance, fetchCurrentEpoch])
   

    const value = useMemo(() => ({ 
        ...state,
        connectAccount: connectAccountHandler,
        disconnectAccount: disconnectAccountHandler,
        fetchEpoch: fetchCurrentEpoch
    }), [
        state, 
